...
class Polynomial:
    ...
    def __mul__(self, other):
        # 使用 FFT 加速多项式乘
        m = len(self.coefficients) + len(other.coefficients)
        n = 1
        while n < m:
            n *= 2
        a = np.fft.fft(self.coefficients, n)
        b = np.fft.fft(other.coefficients, n)
        c = np.fft.ifft(a * b).real  # 取实部作为结果        
        result = np.rint(c).astype(int)  # 四舍五入取整        
        return Polynomial(result)
